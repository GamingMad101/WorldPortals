...
local function ShouldRender( portal, exitPortal, plyOrigin )
	...
	distance = plyOrigin:Distance( portal:GetPos() )
	local disappearDist = portal:GetDisappearDist()

	if (disappearDist>=0 and distance<=disappearDist) then return false end

	if !IsValid( exitPortal ) then return false end

	if !portal:GetShouldDrawNextFrame() then return false end

	
	-- don't render if the player is behind the portal
	-- Currently disabled so you can use a cube
...
hook.Add( "RenderScene", "WorldPortals_Render", function( plyOrigin, plyAngles )
	...
	for _, portal in pairs( portals ) do
		...
		if !ShouldRender( portal, exitPortal, plyOrigin ) then continue end
		...
		render.SetRenderTarget( portal:GetTexture() )
			...
			// New code
			if( !GetPortalSide( portal ) ) then
				render.PushCustomClipPlane( exitPortal:GetForward(), exitPortal:GetForward():Dot(exitPortal:GetPos() - (exitPortal:GetForward()) ) )
			else
				render.PushCustomClipPlane( -exitPortal:GetForward(), -(exitPortal:GetForward()):Dot(exitPortal:GetPos() + (exitPortal:GetForward()) ) )
			end
...
-- Returns true if player is infront of portal, false if player is behind it
function GetPortalSide(portal)
	local portalPos = portal:GetPos()
	local portalNor = portal:GetForward()
	local playerPos = LocalPlayer():GetPos()


	local relPos	= playerPos - portalPos

	local dotProduct = portalNor:Dot(relPos)
